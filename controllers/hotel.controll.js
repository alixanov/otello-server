...
const updateProduct = async (req, res) => {
     try {
          const { id } = req.params;
          const { rasm, icon, nomi, manzil, malumoti, narxi,joylashuv,star, wifi, parking, breakfast, pool, gym } = req.body;

          // Янгиланишни амалга ошириш
          const updatedProduct = await HotelSchema.findByIdAndUpdate(
               id,
               { rasm, icon, nomi, manzil, malumoti, narxi,joylashuv,star, wifi, parking, breakfast, pool, gym },
               { new: true }
          );
